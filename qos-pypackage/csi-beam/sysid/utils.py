...
class Beam(object):
    def __init__(self, pos, Im, E, m_bar):
        """Create a simply supported beam object
        Define the beam with the mass `m` at each dof,
        stiffness `k`, and the number of sensors representing # of beam segments, see figure below.


                nd1         nd2       nd3.........                                              ndn-1    ndn
        ...
        ---------
        n : int
            # of nodes/dofs for only vertical deflection (removing the 2 support nodes)
        K : float
        ...
        self.pos=pos
        self.n = len(pos)-2
        self.L = np.zeros((len(pos)-1),float)
        for ii in range(self.n + 1):
            self.L[ii] = pos[ii + 1] - pos[ii]
        self.Im = Im
        self.E = E
        self.m_bar =m_bar
        self.K=self._get_K()
    ...
    def _get_K(self):
        n= self.n
        E, I, L = self.E, self.Im, self.L
        K = np.zeros((n+2, n+2), float)

        for i in range(n+1):
            ke = np.array([[12 * E * I / L[i] ** 3, -12 * E * I / L[i] ** 3],
                           [-12 * E * I / L[i] ** 3, 12 * E * I / L[i] ** 3]])
    ...
    def _get_M(self):
        n= self.n
        m_bar, L = self.m_bar, self.L
        M = np.zeros((n+2, n+2), float)
        for i in range(n+1):
            me = m_bar * L[i] / 420 * np.array([[156, 54],
                                             [54, 156]])
    ...
    def get_natural_frequency(self, r):
        ...
        K=self.K
        M=self.M
        value,vector=linalg.eig(K,M)
        #omega=np.sort(np.sqrt(np.diag(value)))
        omega=np.sort((np.sqrt(value)).real)
        #omega = np.sort(omega)
        return omega[r-1]
    ...
    def get_mode_shape(self, r):
        ...
        1darray
            ...
            to have unit length.
        """
        value, vector = linalg.eig(self.K, self.M)
        value = np.array([(np.sqrt(value)).real])
        x = np.concatenate((value.T, vector.T), axis=1)
    ...
    def simulate(self, t, F=None, d0=None, v0=None):
        ...
        sys = signal.StateSpace(*self.get_state_space_matrices())
        U = np.zeros((t.size, n)) if F is None else F.T

        _, y, _ = signal.lsim(sys, U, t, X0=x0)
        y = y.T
        A = y[:n, :]
        V = y[n:2*n, :]
        D = y[2*n:, :]
